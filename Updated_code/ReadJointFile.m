function data = ReadJointFile(startpath,rootfile)

% Read both fleezers and old trap files. Modified from ReadMattFile_v8.
% Added ot field to data. Should not affect any fleezer data.
% 150429 kw
...
elseif data.version >= 9
    ...
    elseif data.datatype == 0 %time series. are these even used yet? KW 150430
        data.oldtrap = fread(fid,1,'float64');
...
%         data.profileY = fread(fid,1,'float64');
        ...
        %next three values are not used yet
        data.oldtrap = fread(fid,1,'float64'); % 1 if old trap, 0 if fleezers.
        for i = 1:3
            fread(fid,1,'float64');
        end
    end
    ...
    if data.datatype == 2 %AOM raster scan
        ...
        data.scanrange = fread(fid,1,'float64');%scan range (MHz or V), sign gives direction
        data.nsteps = fread(fid,1,'float64');%number of frequency (voltage for old trap) steps per scan
    ...
    %now read data
    if data.oldtrap
        D = fread(fid,inf,'float32','ieee-be');
    else
    ...
    if data.oldtrap
        F = fread(fid,inf,'float32','ieee-be');
    else
